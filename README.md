...
The best validation performance in the experiments was achieved by **Model 10**.
...
Batch size had a significant effect on model performance.

Using a smaller batch size, particularly:

```text
Batch Size = 32
```

produced much better performance than using a very large batch size.

---

### 2. Effect of Activation Function

The experiments showed that **ReLU and Tanh** performed better than Sigmoid for the hidden layers.

ReLU was particularly effective when used in the hidden layers.

---

### 3. Effect of Hidden Layers

Adding a hidden layer improved the performance of the neural network.

However, increasing the number of hidden layers beyond two did not result in a significant improvement.
...
Increasing the number of epochs can improve training performance.

However, training for too many epochs can lead to **overfitting**.

In the experiments, the deeper model trained for 100 epochs achieved very high training accuracy, but the validation accuracy did not improve significantly.
